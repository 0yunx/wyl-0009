...
游戏中的碰撞检测分为**宽相**（Broad Phase）和**窄相**（Narrow Phase）两阶段：

1. **宽相（快速排除）**：先用圆形碰撞做距离检测，两个实体包围圆不相交则直接跳过。这一步是 O(1) 的距离计算，避免了绝大多数不必要的 SAT 运算。

2. **窄相（精确检测）**：
   - **子弹 vs 敌人**：构建子弹矩形多边形 + 敌人 n 边形多边形 → SAT 精确碰撞
   - **玩家 vs 敌人**：构建玩家五边形飞船轮廓 + 敌人 n 边形 → SAT 精确碰撞
   - **玩家 vs Boss 子弹**：玩家多边形 vs 子弹圆形 → `polygonCircleHit` 精确检测
   - **玩家 vs 道具**：圆形 vs 圆形（道具本身为圆形，无需多边形）

### 多边形构建与缓存

- **飞船多边形**：根据飞船位置动态构建五边形轮廓（三角形机头 + 双翼），位置变化小于 2px 时复用缓存
- **敌人多边形**：根据敌人类型生成不同顶点数的不规则 n 边形（小陨石 7 顶点、中型 9 顶点、Boss 12 顶点），旋转变化小于 4.6° 且位移小于 2px 时复用缓存
- **子弹多边形**：简单的矩形四顶点

### 自适应碰撞降级

当设备性能不足时，系统会自动将碰撞检测从 SAT 多边形降级为纯圆形碰撞：

| 条件 | 行为 |
|------|------|
| FPS 低于 32 | 自动切换为圆形碰撞（`quality: 'low'`） |
| FPS 恢复到 48 以上 | 自动恢复 SAT 精确碰撞（`quality: 'high'`） |
| 碰撞精度降级时 | HUD 左下角显示黄色 ⚠「碰撞精度降级」图标 |

### 碰撞质量设置

在设置面板的「碰撞质量」选项中，玩家可选择三种模式：

| 选项 | 说明 |
|------|------|
| **高精度SAT** | 始终使用 SAT 多边形碰撞，不自动降级（低配设备可能卡顿） |
| **自动适配** | 根据实时 FPS 自动切换 SAT/圆形碰撞（默认） |
| **仅圆形** | 始终使用圆形碰撞，性能最优但碰撞精度略低 |

> 注：圆形碰撞使用包围圆代替真实形状，检测范围略大于视觉轮廓（约 5%~15%），可能导致"看似没碰到但判定命中"的情况。
